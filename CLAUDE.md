...
`sudo` is required to read the system TCC.db, Managed Preferences and the GSA logs; the script fails early (`id -u` check) if it is not run as root. It `chown`s the result back to `$SUDO_USER` so the files can be opened without root afterwards.

On a work machine, pass the company name and project/codenames as arguments — they are the one identifier class the script cannot derive, so without them they stay in cleartext. With no terms the script warns; `ALLOW_NO_TERMS=1` silences that on a personal machine.

The result is written to `$AUDIT_ROOT/audit_<TS>/` (default `$HOME/bigbro_audit`, overridable with the `AUDIT_ROOT` environment variable), one subfolder per run (`TS` = `YYYYMMDD_HHMM`). The per-run subfolder is set to `700`. Permission/ownership changes are scoped to what the run creates (the subfolder always, the root only if the script created it), so `AUDIT_ROOT` pointed at an existing populated directory is not recursively re-owned — point it at a dedicated or non-existent path.
...
9. `09_agents_network` — all system extensions (content filters from any vendor), third-party LaunchDaemons/Agents, PrivilegedHelperTools, running non-Apple services, known EDR/MDM apps, DNS resolver, proxy/PAC, `/etc/hosts`, `/etc/resolver`, admin accounts.
...
- The GSA tunnel is auto-detected in file 7 (Block D) by going through all `utun` interfaces and counting routes whose Netif column points at the interface. An interface with routes beyond its own link is a tunnel that actually carries traffic. For IPv6 the count and the default-route "full tunnel" flag exclude link-local/multicast routes (gateway/destination starting `fe80`/`ff0x`): macOS installs a link-local IPv6 default on every utun, so without that filter every idle utun would look like a full tunnel. Only a default route whose gateway is the interface itself counts. macOS has no clean mapping from interface to owning process, so routing is the reliable signal. `GSA_TUN_V4` / `GSA_TUN_V6` are only optional confirmation addresses (they flag a utun as GSA if they match), not the primary detection. No company-specific values are hardcoded.
- Positional arguments are the user's sensitive/organisation terms, collected once into `TERMS`. They are used in two places: the file 7 root-CA grep (to spot a company TLS-inspection CA) and the redaction pass. There is no separate `ORG_NAME` — passing a term as an argument both searches for it and redacts it. In file 7 the terms are searched as fixed strings (`grep -iF` per term), NOT spliced into the `grep -iE` regex — a term with regex metacharacters would otherwise break the search and the `|| echo` fallback would report a false all-clear, masking a real MITM CA.
...
- A redaction pass runs over the result files before `chown` (default on, `REDACT=off` to disable). It masks structured tokens by regex first (emails, JWTs, GUIDs in three forms — dashed 8-4-4-4-12, underscore-separated as in the Intune SCEP `ModelName`/`LogicalName` identifier, and compact 32-hex, plus MACs), then the literal terms, so a token like an email redacts whole to `[EMAIL]` before any term touches it. Literal terms come in two lists: the auto-derived host identifiers (username, full name via `dscl ... RealName`, computer name, local hostname) are matched with word boundaries so a short/generic name does not substring-mask unrelated text or the placeholders; the long derived ids (serial, UUID) plus the user's `TERMS` are matched greedily as substrings (the placeholder words `GUID/EMAIL/MAC/JWT/REDACTED` are filtered out of that list so a term cannot eat a placeholder). It is a single `perl -i` process over `*.txt` and `*.xml` so the GUID map (`%g`) is shared across files: each distinct GUID becomes a distinct `[GUID-N]`, the same GUID maps the same everywhere, and all three forms of one GUID share a placeholder (separators stripped in the key). The dashed and underscore patterns use hex lookarounds, not `\b`, because `\b` does not fire when the id is glued to a word char (`AC_1de7ad45…`, `LogicalName_31bda227…`) and would leak it. `whats_up_prompt.md` is excluded from the glob (and copied in after redaction) so the prompt stays intact. IP addresses (IPv4 and IPv6, local and public) are intentionally not masked — masking them would gut the tunnel/route output. After the pass, a verification step prints the masked counts (and warns if the serial/UUID lookup came back empty), then re-scans the output for residual emails/GUIDs/MACs/literal terms, warning with `file:line` of anything left.
